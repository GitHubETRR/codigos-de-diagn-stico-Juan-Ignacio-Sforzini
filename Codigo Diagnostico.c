...
int main()
{
    ...
    while (continuar == 'S' || continuar == 's') {
        ...
        scanf(" %c", &continuar);  // Agregar espacio antes del %c
    }
    
    printf("Estadísticas de los vendedores:\n");  // Corregido el printf
    for (int i = 0; i < NUM_VENDEDORES ; i++) {
        printf("Vendedor %s %s, pares vendidos %d\n", vendedores[i].nombre, vendedores[i].apellido, vendedores[i].paresVendidos);  // Corregido el printf
    }
...
void datosVendedor(Vendedor vendedores[NUM_VENDEDORES]) {
    for (int i=0; i < NUM_VENDEDORES; i++) {
        ...
        scanf(" %c", &vendedores[i].genero);  // Espacio antes de %c
